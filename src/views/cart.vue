<template>
  <div class="cart">
    <orderHearder
      title="我的购物车"
      content="温馨提示：产品是否购买成功，以最终下单为准哦，请尽快结算"
    ></orderHearder>
    <div class="wrapper">
      <div class="container">
        <div class="cart-box">
          ...
          <ul class="cart-item-list">
            <li class="cart-item" v-for="(item, index) in list" :key="index">
              ...
              <div class="item-del" @click="deleteItem(item)"></div>
            </li>
          </ul>
        </div>
        <div class="order-wrap clearfix">
          <div class="cart-tip fl">
            ...
            >件商品，已选择<span>{{ checkList.length }}</span
            >件
          </div>
          <div class="total fr">
...
import { getCartList, toggleAll, updateCart, deleteItem } from "@/api/cart.js";
export default {
  components: {
    orderHearder
  },
  ...
  methods: {
    ...
    updateCart(item, type) {
      ...
      updateCart(item.productId, obj).then(data => {
        this.renderData(data);
      });
    },
    getCartList() {
    ...
    renderData(data) {
      ...
      this.checkList = data.cartProductVoList.filter(
